...
namespace
{
    const std::string delimeter_key_value = "=";
    const std::string delimeter_new = "&";
}
...
namespace HandleProtocol
{
    ...
    std::map<std::string, std::string> decode(const std::string& data)
    {
        ...
        std::getline(stream_str, parse_line, ::delimeter_new[0]);
        client_info[ProtocolConst::TYPE] = parse_line;

        // get other key and values of protocol
        while (std::getline(stream_str, parse_line, ::delimeter_new[0]))
        {
